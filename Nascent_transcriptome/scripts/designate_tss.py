...
for sample in sample_list:
    ...
    counts['fc_plus'] = (pd.to_numeric(counts.plus.shift(-1)) + pd.to_numeric(counts.plus.shift(-2)) + 2) / (pd.to_numeric(counts.plus.shift(1)) + pd.to_numeric(counts.plus.shift(2)) + 2)
    counts['fc_minus'] = (pd.to_numeric(counts.minus.shift(1)) + pd.to_numeric(counts.minus.shift(2)) + 2) / (pd.to_numeric(counts.minus.shift(periods = -1)) + pd.to_numeric(counts.minus.shift(periods = -2)) + 2)
