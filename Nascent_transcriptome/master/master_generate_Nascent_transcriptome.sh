...
#Creating BED file for multiBamSummary
touch ${files_dir}mm10_200bs.bed
while IFS=$'\t' read -r chr len
do
   i=0
   while [ $i -lt $len ]
   do
     ...
     echo -e "$chr\t$s\t$e" >> ${files_dir}mm10_200bs.bed
   done
done < ${files_dir}mm10_chrom_sizes.txt
...
do
  ...
  grep 'tss' ${data_dir}$day\_XX_plus_raw.bed | bedtools intersect -v -a - -b ${files_dir}GENCODE_tss_plus.bed | sed 's/$/\t+\t.\t./' - \
  ...
  grep 'tss' ${data_dir}$day\_XX_minus_raw.bed | bedtools intersect -v -a - -b ${files_dir}GENCODE_tss_minus.bed | sed 's/$/\t-\t.\t./' - \
