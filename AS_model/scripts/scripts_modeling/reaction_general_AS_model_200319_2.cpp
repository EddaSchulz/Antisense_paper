...
int prom_rep (int & overlap, int &dima_prior, int &dimb_prior, int &dimb_post, deque<int> &a_gene, deque<int> &b_gene, 
        int &a_prom, int &b_prom, double *p, boost::random::uniform_real_distribution<> &dist_real) {
	if (dima_prior==0) {
		if (b_gene[dimb_post]==1){
			//cout << "a_prom = " << a_prom << endl;
			if (a_prom==0) { //we could also leave this loop out, then promoter can be repressed again by passing RNAP even if it has not been completely reactivated yet
...
int elongation (bool &apol_paused, bool &bpol_paused, int &overlap, int &dima_prior, int &dimb_prior, int &dimb_post, deque<int> &a_gene, deque<int> &b_gene, int &sum_ap, int &sum_bp, 
        ...
        vector<int> &all_a, int &temp_sum_ar, int &temp_sum_br, int &temp_sum_ap, int &temp_sum_bp, int &temp_sum_pA, int &temp_sum_pB, int &a_prom, int &b_prom, 
        boost::random::uniform_real_distribution<> &dist_real, int &step, double *p)
{
    ...
    a_gene.pop_back();
    //cout << "a_prom before 1st prom rep= " << a_prom << endl;
    if (p[2]>0)  {
    ...
    if (p[3]>0) {
		prom_rep(overlap, dima_prior, dimb_prior, dimb_post, a_gene, b_gene, a_prom, b_prom, p, dist_real);
	}
	//cout << "a_prom after 1st prom rep= " << a_prom << endl;
    // Pol on B move backward
    b_gene.push_back(bpol_ini);
    sum_bp = sum_bp - b_gene.front() + b_gene.back();
    b_rna = b_rna + (int)b_gene.front();
    b_gene.pop_front();
    //cout << "a_prom before 2nd prom rep= " << a_prom << endl;
    if (p[2]>0)  {
		SDI(apol_paused, bpol_paused, overlap,dima_prior,dimb_prior,dimb_post,a_gene,b_gene,sum_ap,sum_bp, p, dist_real);
	}
    if (p[0]>0) {
	...
	//cout << "a_prom after 2nd prom rep= " << a_prom << endl;  
    all_a[step] = a_rna;
    ...
    temp_sum_pA = temp_sum_pA + a_prom;
    temp_sum_pB = temp_sum_pB + b_prom;
...
void mexFunction(int nhls, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int dim_ap, dim_bp, a_rna, b_rna, a_pr, b_pr, a_prom, b_prom, overlap, dima_prior, dimb_prior, dima_post, dimb_post, off_states;
    ...
    //ON:a_prom=0, Off1:a_prom=1,..., Offn: a_prom=n
    a_prom = mxGetScalar(prhs[9]);
    b_prom = mxGetScalar(prhs[10]);
    ...
    vector<double> rx(12), cum_rx(12);
    vector<int> all_a(n_steps);
    
    int m = 0, l = 0, temp_sum_ar = 0, temp_sum_br = 0, temp_sum_apr = 0, temp_sum_bpr = 0, temp_sum_ap = 0, temp_sum_bp = 0, temp_sum_pA = 0, temp_sum_pB = 0;
    all_a[0] = a_rna;
    aro[l] = a_rna;
    bro[l] = b_rna;
    apro[l] = a_pr;
    ...
    for (step=0; step<n_steps; step++)
        {
			//if avg time in off state =0, promoter is always ON
			if (k[4]==0) {a_prom=0;}
			if (k[5]==0) {b_prom=0;}			
		////Gillespie reactions
        t_next = 0;
        ...
        while (t_next<v) {
           //?? rx[0] = p[0]*(a_prom==0)*(b_gene.front()==0); // initiation event on Gene A
           //?? indexing?? rx[1] = p[1]*(b_prom==0)*(a_gene.back()==0); ; // initiation event on Gene B
           //RNAP goes from unbound -> paused -> initiation ready -> elongated; while it is in paused state it can spontaneously fall off
            if (dima_prior==0 & dist_real(gen)<p[1]) {
            	rx[0] = k[0]*(a_prom==0)*(apol_ini==0)*(apol_paused==0)*(b_gene[dimb_post]==0);
            }
            else {
            	rx[0] = k[0]*(a_prom==0)*(apol_ini==0)*(apol_paused==0);
            }
            if (dimb_prior==0 & dist_real(gen)<p[1]) {
            	rx[1] = k[1]*(b_prom==0)*(bpol_ini==0)*(bpol_paused==0)*(a_gene[dima_prior+overlap-1]==0);
            } else {
            	rx[1] = k[1]*(b_prom==0)*(bpol_ini==0)*(bpol_paused==0);
            }
            rx[2] = k[2]*a_rna; //A RNA degradation
            rx[3] = k[3]*b_rna; //B RNA degradation
            //if (p[4]>0 & a_prom>0 & b_gene[dimb_post]==0) {
			if (p[4]>0 & a_prom>0) {
				rx[4] = p[4]/k[4]*(a_prom>0)*(b_gene[dimb_post]==0); //Reactivation of pA from Off_i to Off_i+1 or from OFFn back to ON (reversal rate = number OFF states/ average time in off state)
				//cout << "rx prom derep =" << rx[4];
			} else {
				rx[4] = 0;
			}
			if (p[4]>0 & b_prom>0) {
				rx[5] = p[4]/k[5]*(b_prom>0)*(a_gene[dima_prior+overlap-1]==0); //Reactivation of pB from Off_i to Off_i+1
			} else {
				rx[5] = 0;
			}
			// Protein production
        	...
        	//cout << "a_prom before elong = " << a_prom << endl;
            ...
            all_a, temp_sum_ar, temp_sum_br, temp_sum_ap, temp_sum_bp, temp_sum_pA, temp_sum_pB, a_prom, b_prom, dist_real, step, p);
            ...
            //cout << "a_prom after elong= " << a_prom << endl;
