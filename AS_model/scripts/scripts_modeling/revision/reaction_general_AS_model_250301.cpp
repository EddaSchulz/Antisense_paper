...
void mexFunction(int nhls, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int dim_ap, dim_bp, a_rna, b_rna, a_pr, b_pr, a_prom_as, b_prom_as, a_prom_b, b_prom_b, overlap, dima_prior, dimb_prior, dima_post, dimb_post, off_states;
    ...
    a_prom_as = mxGetScalar(prhs[9]);
    b_prom_as = mxGetScalar(prhs[10]);
    a_prom_b = mxGetScalar(prhs[11]);
        ...
        while (t_next<v) {
           ...
           //RNAP goes from unbound -> paused -> initiation ready -> elongated; while it is in paused state it can spontaneously fall off
            if (dima_prior==0 & dist_real(gen)<p[1]) {
            	rx[0] = k[0]*((a_prom_as+a_prom_b)==0)*(apol_ini==0)*(apol_paused==0)*(b_gene[dimb_post]==0);
            }
            else {
            	rx[0] = k[0]*((a_prom_as+a_prom_b)==0)*(apol_ini==0)*(apol_paused==0);
            }
            if (dimb_prior==0 & dist_real(gen)<p[1]) {
            	rx[1] = k[1]*((b_prom_as+b_prom_b)==0)*(bpol_ini==0)*(bpol_paused==0)*(a_gene[dima_prior+overlap-1]==0);
            } else {
            	rx[1] = k[1]*((b_prom_as+b_prom_b)==0)*(bpol_ini==0)*(bpol_paused==0);
            }
			...
			if ((a_prom_as+a_prom_b)==0) {// transition to basal OFF state 
				rx[12] = 1/k[16];
			} else {
				rx[12] = 0;
			}
			if ((b_prom_as+b_prom_b)==0) {// transition to basal OFF state 
				rx[13] = 1/k[17];
			} else {
				rx[13] = 0;
			}
			if (a_prom_b>0) {// transition to from basal OFF to ON state 
				rx[14] = 1/k[14];
			} else {
				rx[14] = 0;
			}
			if (b_prom_b>0) {// transition to from basal OFF to ON state 
				rx[15] = 1/k[15];
			} else {
				rx[15] = 0;
			}
