...
case_free_plus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_case_free_plus_as.txt"), header = FALSE, skip = 3)
colnames(case_free_plus) <- paste0("Case_", seq(1, 12))

case_free_minus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_case_free_minus_as.txt"), header = FALSE, skip = 3)
colnames(case_free_minus) <- paste0("Case_", seq(1, 12))

case_overlap_plus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_case_overlap_plus_as.txt"), header = FALSE, skip = 3)
colnames(case_overlap_plus) <- paste0("Case_", seq(13, 20))

case_overlap_minus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_case_overlap_minus_as.txt"), header = FALSE, skip = 3)
colnames(case_overlap_minus) <- paste0("Case_", seq(13, 20))

#Reads all the different scaled count files for controls
controls_free_plus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_controls_free_plus_as.txt"), header = FALSE, skip = 3)
colnames(controls_free_plus) <- paste0("controls_", seq(1, 12))

controls_free_minus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_controls_free_minus_as.txt"), header = FALSE, skip = 3)
colnames(controls_free_minus) <- paste0("controls_", seq(1, 12))

controls_overlap_plus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_controls_overlap_plus_as.txt"), header = FALSE, skip = 3)
colnames(controls_overlap_plus) <- paste0("controls_", seq(13, 20))

controls_overlap_minus <- read.delim(paste0(count_dir, "XX_", day, "_3prime_controls_overlap_minus_as.txt"), header = FALSE, skip = 3)
colnames(controls_overlap_minus) <- paste0("controls_", seq(13, 20))

#Binds all the count data in a single dataframe
case_counts_plus <- cbind(case_minus_ids, case_free_plus, case_overlap_plus)  %>% 
  pivot_longer(-1, names_to = "bin", values_to = "count") %>% 
  separate(bin, c("group", "bin"), sep = "_")

case_counts_minus <- cbind(case_plus_ids, case_free_minus, case_overlap_minus)  %>% 
  pivot_longer(-1, names_to = "bin", values_to = "count") %>% 
  separate(bin, c("group", "bin"), sep = "_")

controls_counts_plus <- cbind(controls_minus_ids, controls_free_plus, controls_overlap_plus)  %>% 
  pivot_longer(-1, names_to = "bin", values_to = "count") %>% 
  separate(bin, c("group", "bin"), sep = "_")

controls_counts_minus <- cbind(controls_plus_ids, controls_free_minus, controls_overlap_minus)  %>% 
  pivot_longer(-1, names_to = "bin", values_to = "count") %>% 
  separate(bin, c("group", "bin"), sep = "_")
...
plot_overlap_as <- computeMatrix_3prime %>%
...
write_delim(output, paste0(fig_dir, "XX_", day, "_line_sig_3prime_wilcoxtest_as.txt", delim = "\t"))
